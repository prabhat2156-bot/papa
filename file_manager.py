...
# ─────────────────────────────────────────────────────────────
# HTML Project Static Hosting
# ─────────────────────────────────────────────────────────────

_HTML_PROJECTS_ROOT = os.environ.get(
    "PROJECTS_ROOT",
    os.path.join(os.path.dirname(os.path.realpath(__file__)), "projects"),
)

@app.route("/html/<int:uid>/<name>/")
@app.route("/html/<int:uid>/<name>/<path:filepath>")
def serve_html_project(uid, name, filepath="index.html"):
    """Serve static HTML project files at /html/<uid>/<project>/"""
    pdir = os.path.join(_HTML_PROJECTS_ROOT, str(uid), name)
    ...
    safe = os.path.realpath(os.path.join(pdir, filepath))
    if not safe.startswith(os.path.realpath(pdir)):
        abort(403)

    target = os.path.join(pdir, filepath)
