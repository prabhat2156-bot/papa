import os, asyncio, logging, time, sys, shutil, zipfile, re, secrets, base64, sqlite3, json as _json
...
def _read_db_backend() -> str:
    """Returns 'mongodb' or 'local'. Checks env var first, then file."""
    # Env var takes priority (set in Render/VPS dashboard)
    env_val = os.getenv("DB_BACKEND", "").strip().lower()
    if env_val in ("mongodb", "local"):
        return env_val
    # Then check file (set by admin panel switch)
    try:
        with open(DB_BACKEND_FILE, "r") as _f:
            v = _f.read().strip()
            return v if v in ("mongodb", "local") else "mongodb"
    except FileNotFoundError:
        # Default: use local if no MONGODB_URI, else mongodb
        return "local" if not os.getenv("MONGODB_URI", "") else "mongodb"
...
class LocalCollection:
    ...
    def _load_all(self) -> list:
        ...
        for row_id, data in rows:
            try:
                result.append((row_id, __json.loads(data)))
            except Exception:
                pass
        return result
    ...
    async def insert_one(self, doc: dict):
        ...
        def _ins():
            conn = sqlite3.connect(self._db_path)
            conn.execute(f"INSERT INTO [{self._name}] (data) VALUES (?)", (__json.dumps(doc, default=str),))
            conn.commit()
            conn.close()
        async with self._lock:
        ...
        async with self._lock:
            ...
            if found_doc is not None:
                ...
                def _upd(fid=found_id, fd=found_doc):
                    conn = sqlite3.connect(self._db_path)
                    conn.execute(f"UPDATE [{self._name}] SET data=? WHERE id=?", (__json.dumps(fd, default=str), fid))
                    conn.commit()
                    conn.close()
                ...
                def _ins_up(nd=new_doc):
                    conn = sqlite3.connect(self._db_path)
                    conn.execute(f"INSERT INTO [{self._name}] (data) VALUES (?)", (__json.dumps(nd, default=str),))
                    conn.commit()
                    conn.close()
...
# ── RAM / concurrency ─────────────────────────────────────────
MAX_CONCURRENT_PROJECTS = int(os.getenv("MAX_CONCURRENT_PROJECTS", "10"))
_process_semaphore: asyncio.Semaphore = None   # initialised in post_init
...
async def start_project_process(uid: int, name: str) -> dict:
    ...
    # Ensure project directory exists before opening log
...
async def _finalize_new_project(update: Update, context: ContextTypes.DEFAULT_TYPE, via_message: bool):
    ...
    # ── Node.js ──
        ...
        os.makedirs(pdir, exist_ok=True)   # ensure dir exists
